...
void purchaseItem(Book books[], int numBooks, const char* query)
{
    ...
    if (numMatches > 0)
    {
        ...
        char confirm;
        printf("\n\nDo you confirm order? (Y/N): ");
        scanf(" %c", &confirm);
        if (confirm == 'Y' || confirm == 'y')
        {
            // Generate a random 6-digit order number
            srand(time(NULL));
            int orderNumber = rand() % 900000 + 100000; // Ensures a 6-digit number
            ...
            if (cartFile != NULL)
            {
                ...
                fclose(cartFile);
                printf("\n\nPurchase confirmation successful. Your order number is %d. You may proceed to pay for item.\n", orderNumber);
            }
            else
            {
